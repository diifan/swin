...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }: { effectOptions?: EffectOptions }) => {
  ...
  useEffect(() => {
    ...
    const distortions: Record<string, any> = {
      ...
      turbulentDistortion: {
        ...
        getJS: (progress: number, time: number): THREE.Vector3 => {
          ...
          const getX = (p: number): number =>
            Math.cos(Math.PI * p * uFreq.x + time) * uAmp.x +
            Math.pow(Math.cos(Math.PI * p * uFreq.y + time * (uFreq.y / uFreq.x)), 2) * uAmp.y;

          const getY = (p: number): number =>
      ...
      deepDistortion: {
        ...
        getJS: (progress: number, time: number): THREE.Vector3 => {
          ...
          const getX = (p: number): number => Math.sin(p * Math.PI * uFreq.x + time) * uAmp.x;
          const getY = (p: number): number =>
    ...
    class App {
      ...
      loadAssets() {
        // SMAA pass removed (replaced by Bloom-only pipeline), so no assets to load.
        return Promise.resolve();
      }

      init() {
        // Pre-compile all scene materials so the composer's effect materials are
        // fully built before EffectComposer.addPass reads their blendMode.alpha.
        // Without this, addPass throws "Cannot read properties of null (reading 'alpha')".
        try {
        ...
        try {
          ...
          // If pass setup still fails (material warm-up race), skip post-processing;
          // the render loop falls back to a direct renderer.render via the catch below.
      ...
      render(delta: number) {
        if (this.passesReady) {
          ...
          // Composer unavailable: render the scene directly (no bloom, but stable).
      ...
      tick() {
        ...
        if (this.hasValidSize) {
          ...
          // Guard render against transient uniform-null errors during shader warm-up.
          try {
            this.render(delta);
            this.update(delta);
          } catch (err) {
            // Swallow once-per-init compile race; subsequent frames are fine.
          }
        }
    ...
    class CarLights {
      ...
      init() {
        ...
        for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
          ...
          let laneX = (carLane * laneWidth - options.roadWidth / 2 + laneWidth / 2) as number;
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    function resizeRendererToDisplaySize(renderer: any, setSize: any): boolean {
    ...
    const options = {
      ...(DEFAULT_EFFECT_OPTIONS as object),
      ...(effectOptions as object),
      colors: { ...((DEFAULT_EFFECT_OPTIONS as EffectOptions).colors as object), ...((effectOptions as EffectOptions).colors as object) },
    } as Record<string, unknown>;
    options.distortion = distortions[options.distortion as string];

    const myApp = new App(container, options);
    appRef.current = myApp;
    // init() may throw a benign one-time error from EffectComposer.addPass while
    // effect materials warm up (postprocessing 6.x). Catch it so the page never
    // surfaces an uncaught exception; the render loop degrades gracefully.
    myApp.loadAssets().then(() => {
      try {
        myApp.init();
      } catch {
        /* material warm-up race — render loop falls back to direct render */
      }
    });
